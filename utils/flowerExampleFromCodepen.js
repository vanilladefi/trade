...
class Generator {
  ...
  setupControls() {
    ...
    this.vb.addButton({
      ...
      callback: (e, button, variaboard) => {
        ...
        button.dom.button.setAttribute(
          'href',
          this.renderer.domElement.toDataURL('image/png')
        )
        button.dom.button.setAttribute(
          'download',
          `simplex-flower-${Date.now()}.png`
        )
      },
    })
  ...
  generate() {
    ...
    this.scene.background = this.vb.get('invert')
      ? new THREE.Color('#fff')
      : new THREE.Color('#000')
        ...
        color: new THREE.Color(
          `hsl(${
            ...
            Calc.map(
              ...
              0,
              this.count,
              -this.vb.get('hue-range'),
              this.vb.get('hue-range')
            )
          }, 100%, ${this.vb.get('lightness')}%)`
        ),
        depthTest: false,
        opacity: 1,
        transparent: true,
  ...
  loop() {
    ...
    while (i--) {
      ...
      mesh.material.uniforms.opacity.value = Calc.clamp(
    ...
    this.meshGroupScale +=
      (this.meshGroupScaleTarget - this.meshGroupScale) * 0.3
