...
As contracts are non-upgradeable and have no owners, there was no choice except pre-selecting the safe-listed tokens. Instead of hand-picking ERC-20s to the safe-list, the Vanilla team decided to use a selection criteria which should be more fair and verifiable. The criteria we picked was as follows - token got safe-listed if it had:
...
Obviously, even if this selection criteria is verifiable and deterministic, it's still an arbitrary criteria and a lot of rightfully safe and honest ERC-20s were excluded. When the governance is installed, this problem will be resolved as safe-list is then no longer immutable.
