...
No. Vanilla defines a _safe-list_ of tokens, which are only ERC-20s that are eligible for profit mining rewards. While trading non-safe-listed tokens is possible, profitable trading with them will not generate VNL.

To know which tokens are eligible to profit mining rewards, check out the _"Profit Mining"_ column in the [Trade page](/trade) - the eligible ones are marked "Yes".

## Why is the safe-list needed?

For security reasons. Safe-listing protects the VNL distribution against maliciously customized ERC-20s.
...
The [_Value-Protection Coefficient_](#what-is-the-value-protection-coefficient-and-why-does-vanilla-use-it) in reward algorithm protects against the unavoidable Uniswap price manipulation by incentivizing the trades in tokens whose prices are more expensive to manipulate (i.e. have more WETH liquidity in Uniswap). However, when price manipulation happens by specially crafted ERC-20, the liquidity-based protections are not helping. Safe-list is a solution to this.

## Why is token X on the safe-list and not token Y?

As contracts are non-upgradeable and have no owners, the safe-list had to be immutable and therefore selected before the deployment. Instead of hand-picking ERC-20s to the safe-list, the Vanilla team decided to use a selection criteria which should be more fair and verifiable. The criteria we picked was as follows - token got safe-listed if it had:
...
For a proper safe-listing criteria, the selected tokens needed to be less prone to price manipulation. This is the reason why the selection criteria emphasizes sufficient liquidity (manipulation gets more expensive) in more than one markets (manipulation gets harder when the ownership is more decentralized).

Obviously, even if this selection criteria is verifiable and deterministic, it's still an arbitrary criteria and a lot of rightfully safe and honest ERC-20s were excluded. When the governance is installed, this problem will be resolved as the governance will be able to change the safe-list.
...
![R_{i}=P_{i}*V_{i}*H_{i}](https://render.githubusercontent.com/render/math?math=R_{i}=P_{i}*V_{i}*H_{i})

where

- `Rᵢ` is the amount of VNL reward the trader gets.
- `Pᵢ` is the amount of Ether profit made in the trade.
- `Vᵢ` is the _Value-Protection Coefficient_
- `Hᵢ` is the _Holding/Trading Ratio (Squared)_
...
Weighted Average Exchange Rate after `i` trades is calculated using two variables, (![E_{i}](https://render.githubusercontent.com/render/math?math=E_{i})) which represents the adjusted sum of WETHs used in trading and (![T_{i}](https://render.githubusercontent.com/render/math?math=T_{i})) which represents the trader's token balance.

When buying ![Q_{token}](https://render.githubusercontent.com/render/math?math=Q_{token})   tokens with ![Q_{eth}](https://render.githubusercontent.com/render/math?math=Q_{eth}), Vanilla updates the variables as:

![T_{i}=T_{i-1}+Q_{token}\\E_{i}=E_{i-1}+Q_{eth}](https://render.githubusercontent.com/render/math?math=\begin{split}T_{i}=T_{i-1}%2BQ_{token}%5C%5CE_{i}=E_{i-1}%2BQ_{eth}\end{split})

When selling ![Q_{token}](https://render.githubusercontent.com/render/math?math=Q_{token})   tokens for ![Q_{eth}](https://render.githubusercontent.com/render/math?math=Q_{eth}), Vanilla calculates the profit as:

![P_{i}=max(Q_{eth}-\frac{Q_{token}*E_{i-1}}{T_{i-1}}, 0)](https://render.githubusercontent.com/render/math?math=P=max(Q_{eth}-\frac{Q_{token}*E_{i-1}}{T_{i-1}},0))

and updates the variables for the next trade as

![\begin{split}T_{i}=T_{i-1}-Q_{token}%5C%5CE_{i}=\frac{E_{i-1}*T_{i}}{T_{i-1}}\end{split}](https://render.githubusercontent.com/render/math?math=\begin{split}T_{i}=T_{i-1}-Q_{token}%5C%5CE_{i}=\frac{E_{i-1}*T_{i}}{T_{i-1}}\end{split})

For reference, the quote for the average purchase price is calculated simply as a ratio of the two variables:

![P_{i}^{avg}=\frac{T_{token}}{T_{eth}}](https://render.githubusercontent.com/render/math?math=P_{i}^{avg}=\frac{T_{n}}{E_{n}})
...
The Coefficient formula is ![V=1-min(\frac{P+L}{W},1)](https://render.githubusercontent.com/render/math?math=V_{i}=1-min(\frac{P_{i}%2BL}{W_{i}},1)) where
...
- `Wᵢ` is the internally tracked WETH reserve size for the Uniswap liquidity pair. This means that trades of high-liquidity tokens will get a higher coefficient and reward than low-liquidity tokens.

For example, making a profit of 10ETH in a token A with 1000ETH reserves would result in `Vᵢ = 0.49`, but making the same profit in a token B with 10000ETH in reserves would result in `Vᵢ = 0.949`. Even if the Ether-denominated profits are the same, the VNL rewards will differ greatly.

To further protect the token value against the more sophisticated market manipulation, the internally tracked `Wᵢ` is updated using Uniswap WETH reserves for the traded token `Uᵢ` with the following rules:

- ![W_{i}=max(W_{i-1},U_{i})](https://render.githubusercontent.com/render/math?math=W_{i}=max(W_{i-1},U_{i})) when buying the token, which means that price manipulators can only manipulate the reserve upwards either by providing liquidity or buying token which drives price up, and
- ![W_{i}=min(W_{i-1},U_{i})](https://render.githubusercontent.com/render/math?math=W_{i}=min(W_{i-1},U_{i})) when selling the token, which means that Uniswap reserves can only be manipulated downwards either by removing liquidity or selling token which drives the price down
...
For each token you trade, Vanilla maintains _Weighted Average Purchase Block_ (WAPB) or (![B_{w,i}](https://render.githubusercontent.com/render/math?math=B_{w,i})).
The WAPB after `i` trades is calculated by using two variables, (![A_{i}](https://render.githubusercontent.com/render/math?math=A_{i})), which represents the token-volume weighted sum of block numbers of the purchases and (![T_{i}](https://render.githubusercontent.com/render/math?math=T_{i})), which represents the trader's token balance, used already in Weighted Average Exchange Rate- calculations.

When buying ![Q_{token}](https://render.githubusercontent.com/render/math?math=Q_{token}) tokens with ![Q_{eth}](https://render.githubusercontent.com/render/math?math=Q_{eth}), Vanilla updates the variable as:

![A_{i}=A_{i-1}%2BQ_{token}B_i](https://render.githubusercontent.com/render/math?math=A_{i}=A_{i-1}%2BQ_{token}B_i), in which ![B_{i}](https://render.githubusercontent.com/render/math?math=B_{i}) represents the block number of the purchase.

When selling ![Q_{token}](https://render.githubusercontent.com/render/math?math=Q_{token}) tokens for ![Q_{eth}](https://render.githubusercontent.com/render/math?math=Q_{eth}), updates the variable as:

![A_{i}=\frac{A_{i-1}T_i}{T_{i-1}}](https://render.githubusercontent.com/render/math?math=A_{i}=\frac{A_{i-1}T_i}{T_{i-1}})

And like _WAER_, the quote for the WAPB is calculated as a ratio of the two variables:

![B_{w,i}=\frac{A_{i}}{T_{i}}](https://render.githubusercontent.com/render/math?math=B_{w,i}=\frac{A_{i}}{T_{i}})

The Holding/Trading Ratio (Squared) is calculated as:

![H_i=\frac{(B_s-B_{w,i})^2}{(B_s-B_f)^2}](https://render.githubusercontent.com/render/math?math=H_i=\frac{(B_s-B_{w,i})^2}{(B_s-B_f)^2})

- ![B_s](https://render.githubusercontent.com/render/math?math=B_s) is the block number, when a sell-transaction happens.
- ![B_f](https://render.githubusercontent.com/render/math?math=B_f) is the first block number of Vanilla system.

The Holding/Trading Ratio (Squared) component is in place to bootstrap usage by incentivizing early participation and longer holding periods.
...
The denominator describes the number of blocks the Vanilla system itself has been operational. The denominator will increase over time as Ethereum block numbers increase and will decrease the ratio.
...
The block numbers of Vanilla are Ethereum block numbers. The first block number is determined when Vanilla is launched.
...
- The profit `Pᵢ` made in ETH determines the theoretical maximum of VNL attributed for a particular trade
- The Value-Protection Coefficient `Vᵢ` and Holding/Trading Ratio (Squared) `Hᵢ` scale the theoretical value downward.

A trader can increase VNL rewards by:
- Making higher percentage returns on his trades or trading larger ETH positions, i.e. making higher absolute ETH returns -> Higher profit component `Pₙ`
- Trading tokens with higher WETH reserves -> Higher Value-Protection Coefficient `Vᵢ`
- Opening trades early as possible -> Higher Holding/Trading Ratio (Squared) `Hᵢ`
- Holding the trades longer -> Higher Holding/Trading Ratio (Squared) `Hᵢ`
