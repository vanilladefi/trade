...
This list may change as new contracts are deployed, or as existing contracts are removed from usage. Vulnerabilities in contracts built on top of the Protocol by third-party developers (such as smart contract wallets) are not in scope, nor are vulnerabilities that require the ownership of an admin key.

The secondary scope of the bug bounty program is for vulnerabilities in the Vanilla Interface code that could likely result in the unauthorized exploitation of users of Vanilla Trading Interface hosted at https://vanilladefi.com/trade.
